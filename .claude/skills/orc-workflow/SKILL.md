...
- Use `permission_mode` on `claude-code` nodes when the default `acceptEdits` is insufficient. Use `full` when the agent needs to run shell commands autonomously. Avoid `bypassPermissions` unless the workflow is fully trusted.
