...
- **`interactive`** — two modes:
  - **`session`** — launches the agent in a tmux session. User attaches via `orc attach`. Requires `agent` field. Used for AI agents or tools that need an ongoing terminal session. Can be combined with `loop` — each loop iteration spawns a new tmux session, and the user re-attaches each time (see §5.4).
...
- **Values:** template variables (`{{node_id.status}}`, `{{node_id.output}}`) and string literals (`'completed'`, `'yes'`)
...
- Downstream nodes follow the standard dependency rule: a node runs if at least one dependency completed; it is skipped only if all dependencies were skipped.
...
YAML is loaded, validated, and deserialized into a `Workflow` model. Template variables are checked against declared inputs. Evaluator references are resolved. `when:` expressions are parsed for syntax errors.
...
- **`skip`** — marks the node as `skipped`. Downstream nodes that depend **only** on skipped nodes are also skipped. A node with multiple dependencies runs if at least one dependency completed — it is skipped only if all dependencies were skipped. `{{node_id.status}}` is available to downstream nodes for branching.
